...
def plot_simplex(df,
    ...
    if scale:
        ...
        c['a'] = 'gene_tpm_parent_5_prop'
        c['b'] = 'gene_tpm_prop'
        c['c'] = 'gene_tpm_parent_3_prop'
    else:
        c['a'] = 'n_tss'
        c['b'] = top
        c['c'] = 'n_tes'
    ...
    # sectors
    if sectors:
        line_dorito(figure, tax, scale, size_dict, **kwargs)
    ...
    tax.left_axis_label("5' parent expression", fontsize=size_dict['label_size'],
                        offset=size_dict['left_label_offset'])
    tax.right_axis_label("Readthrough expression", fontsize=size_dict['label_size'],
                         offset=size_dict['right_label_offset'])
    tax.bottom_axis_label("3' parent expression", fontsize=size_dict['label_size'],
        ...
        for j in range(0, scale+1):
            for k in range(0, scale+1):
                if i+j+k == scale:
                    ...
                    if i != scale:
                        temp = temp.loc[(temp[c['a']]*scale>=i)&(temp[c['a']]*scale<i+1)]
                    else:
                        temp = temp.loc[(temp[c['a']]*scale>=i)&(temp[c['a']]*scale<=i+1)]
                    if j != scale:
                        temp = temp.loc[(temp[c['b']]*scale>=j)&(temp[c['b']]*scale<j+1)]
                    else:
                        temp = temp.loc[(temp[c['b']]*scale>=j)&(temp[c['b']]*scale<=j+1)]
                    n = len(temp.index)
                    hm_dict[i,j] += n
...
def line_dorito(figure,
    ...
    # 'gene_tpm_parent_5_prop': tss,
    #           'gene_tpm_prop': splicing,
    #           'gene_tpm_parent_3_prop': tes}

    # splicing line
    tax.horizontal_line(beta, linewidth=linewidth,
                     color=c_dict['gene_tpm_prop'],
                     linestyle='--')

    # tss
    tax.right_parallel_line(alpha, linewidth=linewidth,
                        color=c_dict['gene_tpm_parent_5_prop'],
                        linestyle='--')

    # tes
    tax.left_parallel_line(gamma, linewidth=linewidth,
                        color=c_dict['gene_tpm_parent_3_prop'],
                        linestyle='--')
...
def get_sector_colors(cats=None):
    ...
    tes = '#052e81'
    splicing = '#b10000'
    c_dict = {'gene_tpm_parent_5_prop': tss,
              'gene_tpm_prop': splicing,
              'gene_tpm_parent_3_prop': tes}
    # order = ['tss', 'splicing', 'tes']
